   ...
   /**
    ...
    hlist.forEach(function(value, index) {
	   ...
	   g.event.addListener(marker, "click", function(){
		   ...
		   markerList = [];
		 /*
			 * //tab 펼침 getTabId(); infowindow.setContent(iw_content);
			 * iw_content.style.display = "block"; infowindow.open(map, this);
			 */
 ...
 * searchBox.setBounds(map.getBounds()); }); // Create markers. var markers =
 * []; // Listen for the event fired when the user selects a prediction and
 * retrieve // more details for that place.
 * searchBox.addListener('places_changed', function() { var places =
 * searchBox.getPlaces();
 * 
 * if (places.length == 0) { return; } // Clear out the old markers.
 * markers.forEach(function(marker) { marker.setMap(null); }); markers = [];
 *  // For each place, get the icon, name and location. var bounds = new
 * google.maps.LatLngBounds(); places.forEach(function(place) { if
 * (!place.geometry) { console.log("Returned place contains no geometry");
 * return; } var icon = { url: place.icon, size: new google.maps.Size(10, 10),
 * scaledSize: new google.maps.Size(25, 25) }; thisStateMarker.setIcon(icon) //
 * Create a marker for each place. markers.push(new google.maps.Marker({ map:
 * map, icon: icon, title: place.name, position: place.geometry.location }));
 * 
 * if (place.geometry.viewport) { // Only geocodes have viewport.
 * bounds.union(place.geometry.viewport); } else {
